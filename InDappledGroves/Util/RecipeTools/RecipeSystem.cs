...
namespace InDappledGroves.Util.RecipeTools
{
    public class IDGRecipeNames : ICookingRecipeNamingHelper
    {
        ...
        public class IDGRecipeLoader : RecipeLoader
        {
            ...
            //            Inputs[i] = new CraftingRecipeIngredient();
            //            Inputs[i].FromBytes(reader, resolver);
            //            Inputs[i].Resolve(resolver, "Sawing Ingredient (FromBytes)");
            //        }
            //    }
        ...
        public class SplitterRecipe : IByteSerializable
        {
            ...
            public void FromBytes(BinaryReader reader, IWorldAccessor resolver)
            {
                ...
                for (int i = 0; i < Ingredients.Length; i++)
                {
                    Ingredients[i] = new ChoppingBlockIngredient();
                    Ingredients[i].FromBytes(reader, resolver);
                    Ingredients[i].Resolve(resolver, "Log Splitter Recipe (FromBytes)");
                }
        ...
        //            Ingredients[i].FromBytes(reader, resolver);
        //            Ingredients[i].Resolve(resolver, "Sawhorse Recipe (FromBytes)");
        //        }
